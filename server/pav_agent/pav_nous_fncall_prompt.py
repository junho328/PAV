...
from pav_agent.pav_schema import ASSISTANT, FUNCTION, SYSTEM, USER, PLANNER, ACTOR, VERIFIER, ContentItem, FunctionCall, Message
    ...
    def preprocess_fncall_messages(self,
        ...
        for msg in copy.deepcopy(ori_messages):
            ...
            if role in (SYSTEM, USER):
                ...
                if fn_call:
                    if (not SPECIAL_CODE_MODE) or (CODE_TOOL_PATTERN not in fn_call.name):  # 일반적인 경우
                        fc = {'name': fn_call.name, 'arguments': json5.loads(fn_call.arguments)}
                        fc = json.dumps(fc, ensure_ascii=False)
                        fc = f'<tool_call>\n{fc}\n</tool_call>'
                    else:   # Code 혹은 tool pattern인 특수한 경우
        ...
        if SPECIAL_CODE_MODE and any([CODE_TOOL_PATTERN in x for x in tool_names]): # Code 혹은 tool pattern인 특수한 경우
            tool_system = FN_CALL_TEMPLATE_WITH_CI.format(tool_descs=tool_descs)
        elif messages and messages[0].role == PLANNER:
            tool_system = FN_CALL_PLANNER_TEMPLATE.format(tool_descs=tool_descs)
        else:   # 일반적인 경우
            tool_system = FN_CALL_TEMPLATE.format(tool_descs=tool_descs)
        
        if messages and messages[0].role == SYSTEM: # role==SYSTEM에 대하여 system prompt에 tool 정보 포함
            # print("system")
            messages[0].content.append(ContentItem(text='\n\n' + tool_system))
        elif messages and messages[0].role == PLANNER: # role==PLANNER에 대하여 system prompt제외, user 정보에 tool 정보 포함
            # print("planner")
            messages[0].content.append(ContentItem(text=f'\n\n{tool_system}'))
            messages[0].role = "user"
        else:       # role!=SYSTEM에 대하여 prompt 앞에 system prompt(tool 정보 포함) 추가
            # print("else")
...
FN_CALL_PLANNER_TEMPLATE = """# Tools
