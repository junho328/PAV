...
class Message(BaseModelCompatibleDict):
    ...
    def role_checker(cls, value: str) -> str:
        if value not in [USER, ASSISTANT, SYSTEM, FUNCTION, PLANNER, ACTOR, VERIFIER]:
            raise ValueError(f'{value} must be one of {",".join([USER, ASSISTANT, SYSTEM, FUNCTION, PLANNER, ACTOR, VERIFIER])}')
        return value
